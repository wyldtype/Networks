...
And Hybrids currently have mainly 2-4:
...
sample_info |> filter(organism == "hyb") |> 
  ...
  table() |> table()
```
By the end of this section, every parental sample should still have at most 3 entries and most should have 3 (6 for hybrids, one for each of the two alleles).
...
cat("Hybrid samples (all 2s and 4s):\n")
...
sample_info |> filter(organism %in% c("cer", "par") & grepl("GS2018", well_flask_ID)) |> 
  select(well_flask_ID) |> table() 
cat("Hybrid samples (should have 6):\n")
sample_info |> filter(organism == "hyb" & grepl("GS2018", well_flask_ID)) |> 
  select(well_flask_ID) |> table()
...
And for hybrid samples: a) most well flask IDs are represented 6 times, and b) no well flask ID is represented more than 6 times for each organism/condition:
